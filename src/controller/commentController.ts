...
export class CommentController {
  ...
  getComments(@Param('targetId') targetId: string, @Query("page", ParseIntPipe) page: number, @Query("size", ParseIntPipe) size: number) {
    this.logger.log(`GET /comments/${targetId} | Fetching comments with page=${page}, size=${size}`);
  ...
  addComment(@Param('targetId') targetId: string, @Body() createCommentDto: Comment) {
    this.logger.log(`POST /comments/${targetId} | Adding comments`);
    if (!validate(targetId)) {
      this.logger.warn(`POST /comments/${targetId} | Invalid ID format`);
      throw new BadRequestException('Invalid ID format');
    }
    return this.commentService.addComment(targetId, createCommentDto);
  }
  ...
  updateComment(@Param('commentId') commentId: string, text: string) {
    this.logger.log(`PUT /comments/${commentId} | Updating comment with text: "${text}"`);
  ...
  deleteComment(@Param('commentId') commentId: string) {
    this.logger.log(`DELETE /comments/${commentId} | Deleting comment`);
    if (!validate(commentId)) {
